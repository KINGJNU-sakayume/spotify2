...
export default function PickCard({ song, onClick, T, F }) {
  ...
  return (
    <button
      ...
      onFocus={() => setHov(true)}
      onBlur={() => setHov(false)}
      style={{ background: T.cardBg, borderRadius: 10, overflow: 'hidden', cursor: 'pointer', transition: 'background 0.2s, transform 0.2s', transform: hov ? 'translateY(-2px)' : 'none', border: `1px solid ${T.cardBorder}`, padding: 0, display: 'block', width: '100%', textAlign: 'left' }}>
      <div style={{ position: 'relative', aspectRatio: '1', overflow: 'hidden' }}>
        ...
        {hov && (
          <div aria-hidden="true" style={{ position: 'absolute', bottom: 8, right: 8, width: 30, height: 30, borderRadius: '50%', background: `linear-gradient(135deg,${song.colors.light},${song.colors.accent})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
